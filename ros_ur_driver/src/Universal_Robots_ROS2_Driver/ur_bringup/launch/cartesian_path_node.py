...
class JointStatesListener(Node):
    ...
    def callbackState(self, msg):
        ...
        if self.next_goal is True:
            ...
            self.current_positions[0], self.current_positions[2] = self.current_positions[2], self.current_positions[0]
            #print(self.current_positions)
            #print(self.joint_positions_target)

            reached_targets = [abs(curr - target) < tolerance for curr, target in zip(self.current_positions, self.joint_positions_target)]
            #print(reached_targets)
        # Comprobar si todas las posiciones articulares están dentro de la tolerancia
